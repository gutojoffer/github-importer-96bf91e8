...
export default function ArenaMiniatures({ pendingMatches, getPlayer, arenaNames }: ArenaMiniaturesProps) {
  ...
  return (
    <div className="space-y-2">
      <p className="font-heading text-[10px] text-muted-foreground tracking-[0.2em] uppercase">Partidas Pendentes</p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
        {pendingMatches.map((m) => {
          ...
          return (
            <div key={m.id} className="dark-panel p-2 flex items-center gap-1.5 text-[10px]">
